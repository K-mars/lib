/* kmarscolor.js by kmarsweb,tk*/
(function () {
function kmarscolor(color, elmnt) {
  if (!(this instanceof kmarscolor)) { return new kmarscolor(color, elmnt); }
  if (typeof color == "object") {return color; }
  this.attachValues(toColorObject(color));
  if (elmnt) {elmnt.style.backgroundColor = this.toRgbString();}
}
kmarscolor.prototype = {
...
function kmarstrim(x) {
  return x.replace(/^\s+|\s+$/g, '');
}
function isHex(x) {
  return ('0123456789ABCDEFabcdef'.indexOf(x) > -1);
}
window.kmarscolor = kmarscolor;

})();

function kmarsSetColorsByAttribute() {
  var z, i, att;
  z = document.getElementsByTagName("*");
  for (i = 0; i < z.length; i++) {
    att = z[i].getAttribute("data-kmars-color");
    if (att) {
      z[i].style.backgroundColor = kmarscolor(att).toRgbString();      
    }
  }
}
